...
import { Cloud, Users, Files, BarChart3, Megaphone, Bot, Menu, X, Home, Shield } from "lucide-react";
import { Link, Outlet, useLocation } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

const adminNav = [
  { label: "الإحصائيات", icon: BarChart3, path: "/admin" },
  { label: "المستخدمون", icon: Users, path: "/admin/users" },
  { label: "الملفات", icon: Files, path: "/admin/files" },
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={cn(
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="font-cyber text-sm font-bold text-destructive">Admin</span>
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="h-16 border-b border-border flex items-center justify-between px-4 bg-card/50">
          ...
          <span className="font-cyber text-sm text-muted-foreground">لوحة تحكم الأدمن</span>
        </header>
